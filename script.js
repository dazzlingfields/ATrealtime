// ================== v4.12 - Realtime Vehicle Tracking with Vehicle Count + Bus Types ==================
...
const occupancyLabels = {
    ...
    5:"Full",6:"Not Accepting Passengers"
};
...
function pairAMTrains(inService, outOfService) {
    ...
    inService.forEach(inTrain => {
        ...
        outOfService.forEach(outTrain => {
            ...
            if(dist <= 200 && speedDiff <= 10 && dist < minDist) {
                minDist = dist;
                bestMatch = outTrain;
            }
        });
...
async function fetchVehicles(){
    ...
    await Promise.all(vehicles.map(async v => {
        ...
        const vehicleLabel = v.vehicle.vehicle?.label || "0";
        const operator = v.vehicle.vehicle?.operator_id || "";
        const vehicleNumber = Number(vehicleLabel);
        ...
        let routeName = "N/A", destination = "N/A", busType = "";
        ...
        // --- Bus type lookup ---
        if(typeKey === "bus" && busTypes && operator && !isNaN(vehicleNumber)) {
