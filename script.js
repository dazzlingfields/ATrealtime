// ================== v4.6b - Real-time Vehicle Tracking (Viewport Optimized) ==================
...
async function fetchVehicles(){
  ...
  // --- viewport bounds + buffer ---
  const bounds = map.getBounds();
  const buffer = 0.02; // ~2km buffer
  const latMin = bounds.getSouth() - buffer;
  const latMax = bounds.getNorth() + buffer;
  const lonMin = bounds.getWest() - buffer;
  const lonMax = bounds.getEast() + buffer;
  ...
  for(const v of vehicles){
    ...
    // Skip vehicles outside viewport + buffer
    ...
    let routeName = "N/A", destination = v.vehicle.trip?.trip_headsign || "N/A";
    ...
    // --- Bus type from JSON ---
    let busType = "";
    if(typeKey==="bus" && routeId){
      for(const model in busTypes){
        const operators = busTypes[model];
        if(operators[operator] && operators[operator].includes(Number(vehicleLabel))){ busType = model; break; }
      }
    }
...
(async function init(){
  ...
  map.on('moveend', fetchVehicles); // update on pan/zoom
})();
