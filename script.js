// ================== v5.0 - Optimized Real-time Vehicle Tracking ==================
...
async function loadAllRoutes(){ 
  ...
  if(json?.data){ 
    json.data.forEach(route => routes[route.id||route.route_id]=route.attributes||route); 
  } 
}
...
async function fetchTripsBulk(tripIds){
  ...
  const batchSize = 10; // reduce API load
...
async function fetchVehicles(){
  ...
  // --- Bulk fetch trip data once per unique trip ---
  ...
  vehicles.forEach(v=>{
    ...
    // Determine destination/headsign
...
(async function init(){
  ...
  // Load all vehicles at start
  await fetchVehicles();

  // Start interval updates
  setInterval(fetchVehicles,15000);

  // Map move just filters displayed markers, does not trigger new fetch
  map.on("moveend", ()=>{ 
    Object.values(vehicleMarkers).forEach(m=>{
      const inBounds = map.getBounds().contains(m.getLatLng());
      const layerKey = Object.keys(layerGroups).find(k=>layerGroups[k].hasLayer(m))||"other";
      if(inBounds){ if(!map.hasLayer(m)) m.addTo(layerGroups[layerKey]); }
      else{ if(map.hasLayer(m)) map.removeLayer(m); }
    });
  });
})();
