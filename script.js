// ================== v4.12 - Realtime Vehicle Tracking with Correct Bus Type ==================
...
async function fetchVehicles() {
  ...
  await Promise.all(vehicles.map(async v => {
    ...
    // --- Bus type lookup ---
    if(typeKey === "bus" && busTypes && operator && vehicleNumber){
      for(const model in busTypes){
        const operators = busTypes[model];
        if(operators[operator] && operators[operator].includes(vehicleNumber)){
          busType = model;
          break;
        }
      }
