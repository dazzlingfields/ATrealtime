// ================== v4.12 - Realtime Vehicle Tracking with Vehicle Count + Bus Model ==================
...
// --- Map setup ---
const map = L.map("map", { zoomControl: false }).setView([-36.8485, 174.7633], 12);
...
async function fetchVehicles(){
  ...
  await Promise.all(vehicles.map(async v=>{
    ...
    const operator = v.vehicle.vehicle?.operator_id||""; // two-letter operator
    const vehicleNumber = Number(vehicleLabel)||0;
      ...
      if(trips[tripId]) destination=trips[tripId].trip_headsign||"N/A";
        ...
        if(tripData){
          ...
          if(marker){ 
            ...
            marker.getPopup().setContent(oldContent.replace(/(<b>Destination:<\/b>\s*)(N\/A)/,`$1${tripData.trip_headsign||"N/A"}`)); 
          }
        }
      });
    }

    // --- Bus type lookup from JSON ---
    ...
    const popupContent=`<b>Route:</b> ${routeName}<br><b>Destination:</b> ${destination}<br><b>Vehicle:</b> ${operator}${vehicleLabel}<br>${busType?`<b>Bus Model:</b> ${busType}<br>`:""}<b>Number Plate:</b> ${licensePlate}<br><b>Speed:</b> ${speed}<br><b>Occupancy:</b> ${occupancy}`;
