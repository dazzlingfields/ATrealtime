// ================== v4.8.5 - Real-time Vehicle Tracking (Bulk Trips + Optimized API) ==================
...
async function fetchTripsBulk(tripIds){
  ...
  const batchSize = 50; // larger batch for optimization
...
(async function init(){
  await loadBusTypes();
  await loadAllRoutes();
  await fetchVehicles(); // initial bulk load
  setInterval(fetchVehicles,15000); // refresh positions

  // Map move only filters markers
